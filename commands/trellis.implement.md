...
12. **Generate manual test plan**:
    ...
    After implementation is complete, generate a comprehensive manual testing guide at `FEATURE_DIR/test-plan.md`.

    **Step 12a: Gather testing context**:
    - Read spec.md for user stories and acceptance criteria
    - Read plan.md for architecture and tech stack details
    - Read quickstart.md (if exists) for integration scenarios
    - Read data-model.md (if exists) for entity relationships
    - Read contracts/ (if exists) for API specifications
    - Review the list of implemented files from the execution
    - Review tasks.md for feature breakdown

    **Step 12b: Generate test-plan.md structure**:

    Create `FEATURE_DIR/test-plan.md` with the following sections:

    ```markdown
    # Manual Test Plan: [FEATURE_NAME]

    **Generated**: [DATE]
    **Feature Epic**: [ROOT_EPIC_ID]
    **Implementation Status**: Complete
    ...
    - [Test data or database setup required]

    ### Setup Steps
    1. [Step-by-step instructions to prepare test environment]
    2. [Include commands to run, files to configure]
    3. [How to verify setup is correct]

    ### Test User Accounts
    - [List any test accounts needed with credentials]
    - [Roles/permissions required for testing]

    ---

    ## Manual Verification Checklist

    Test each feature systematically. Check the box when test passes.
    ...
    **Test Case 1.2: [Scenario Name]**
    [Repeat structure]

    ### Phase 2: [PHASE_NAME]
    [Continue for all phases and user stories]
    ...
    **Steps**:
    1. [Detailed step with expected UI/behavior]
    2. [Include specific buttons, fields, navigation]
    3. [Be explicit about expected responses]

    **Expected Outcome**:
    - [Bullet points of what success looks like]
    - [Include UI states, data changes, notifications]

    **Verification**:
    - [ ] Check [specific element/behavior]
    - [ ] Verify [data/state change]
    - [ ] Confirm [system response]

    ### Scenario 2: [Alternative Path Name]
    [Repeat structure for all major workflows]

    ---

    ## Notes

    [Space for tester notes, observations, issues found]

    ---

    **Test Plan Generated by**: `/trellis.implement`
    **Last Updated**: [TIMESTAMP]
    ```

    **Step 12c: Populate with actual implementation details**:
    - Map user stories from spec.md to test scenarios
    - Extract acceptance criteria as test checkpoints
    - Include actual file paths and component names
    - Add specific API endpoints from contracts/ (if exists)
    - Reference actual URLs/routes from implementation
    - Include real field names, button labels from code
    - Add setup commands from plan.md (dev server, database, etc.)
    - Create realistic test data based on data-model.md

    **Step 12d: Tailor to tech stack**:
    - For web apps: Include browser DevTools checks
    - For APIs: Provide curl/Postman examples
    - For CLIs: Include command examples
    - For mobile: Note device/simulator requirements
    - For databases: Include query verification steps

    **Step 12e: Write test-plan.md**:
    ```bash
    # Create the test plan file
    Write test-plan.md to FEATURE_DIR/test-plan.md
    ```
    ...
    **Display confirmation**:
    ...
    Test plan includes:
    • [N] user story test scenarios
    • [N] edge case validations
    • [N] integration test flows
    • [N] verification checkpoints
